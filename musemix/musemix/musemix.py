...
class MuseMixROSAudio(Node):
  def __init__(self):
    super().__init__('musemix')
    ...
    this_share_directory = get_package_share_directory('musemix')
    this_base_directory = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(this_share_directory))))
    this_src_directory = os.path.join(this_base_directory,"src","musemix")
    ...
    self.muse_ring_in_int.extend([0.0]*(self.segment_size))
  ...
  def muse_callback(self):
    ...
    input_win_int = torch.tensor([self.muse_ring_in_int.popleft() for _i in range(self.segment_size)],device=self.device).unsqueeze(0)
    ...
    noisy_amp_int, noisy_pha_int, noisy_com_int = self.mag_pha_stft(input_win_int, self.n_fft, self.hop_size, self.win_size, self.compress_factor)
    amp_g, pha_g, com_g = self.muse(noisy_amp.to(self.device, non_blocking=True), noisy_pha.to(self.device, non_blocking=True),noisy_amp_int.to(self.device, non_blocking=True), noisy_pha_int.to(self.device, non_blocking=True))
  ...
  def jackaudio_callback(self, msg):
    ...
    self.muse_ring_in.extend(data_est)
    self.muse_ring_in_int.extend(data_int)
...
def main(args=None):
  ...
  musemixrosaudio = MuseMixROSAudio()
  rclpy.spin(musemixrosaudio)

  # Destroy the node explicitly
  # (optional - otherwise it will be done automatically
  # when the garbage collector destroys the node object)
  musemixrosaudio.destroy_node()
  rclpy.shutdown()
