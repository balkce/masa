...
void apply_weights (jack_ringbuffer_t **in, rosjack_data **out){
    ...
    for(i_local = 0; i_local < number_of_microphones; i_local++){
        ...
        //storing reference microphone for interference estimation later
        if(i_local == 0){
            for(j_local = 0; j_local < fft_win; j_local++){
                y_time_int[j_local] = x_time[j_local];
            }
        }
    ...
    for(unsigned int j = 1; j < fft_win; j++){
        ...
        double ref_phase = arg(in_fft(0,j));
        ...
        if(this_freq >= freq_min && this_freq <= freq_max){
            ...
            if(this_mag > freq_mag_threshold){
              ...
              y_fft[j] = std::complex<double>(0.01*cos(ref_phase),0.01*sin(ref_phase));
            }
        }else{
            //y_fft[j] = in_fft(0,j)*0.01;
            y_fft[j] = std::complex<double>(0.01*cos(ref_phase),0.01*sin(ref_phase));
    ...
    for(j_local = 0; j_local < fft_win; j_local++){
      y_time_int[j_local] *= (double)fft_win;
      y_time_int[j_local] -= y_time[j_local];
    }
    overlap_and_add_prepare_output(y_time_int,out[1]);
...
int main (int argc, char *argv[]) {
    ...
    y_time_int = (std::complex<double>*) fftw_malloc(sizeof(std::complex<double>) * fft_win);
