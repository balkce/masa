...
class Solver(object):
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            estimate, noise_estimate = extract_interf(self.dmodel(noisy_interf))
            ...
            with torch.autograd.set_detect_anomaly(True):
                with autocast('cuda',enabled=self.use_amp):
                    ...
                    if self.args.stft_loss:
                        ...
                        sc_loss, mag_loss = self.mrstftloss(noise_estimate.squeeze(1), noise.squeeze(1))
                        enh_loss_n = self.args.stft_loss_weight * (sc_loss + mag_loss)
                        loss += enh_loss_n
                    else:
                        enh_loss = torch.Tensor([0]).to(device='cuda')
                        enh_loss_n = torch.Tensor([0]).to(device='cuda')
            ...
            total_enhancement_loss += enh_loss.item() + enh_loss_n.item()
            logprog.update(loss=format(total_loss / (i + 1), ".5f"))
            # Just in case, clear some memory
            del loss, estimate, noise_estimate, enh_loss, enh_loss_n
