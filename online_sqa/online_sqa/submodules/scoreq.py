...
class Scoreq():
    ...
    def predict_data(self, test_wave_raw, ref_wave_raw=None):
        """Makes predictions on raw data."""
        if self.use_onnx:
            return self._predict_data_onnx(test_wave_raw, ref_wave_raw)
        else:
            return self._predict_data_pytorch(test_wave_raw, ref_wave_raw)

    def _predict_data_onnx(self, test_wave_raw, ref_wave_raw=None):
        """Prediction using the ONNX model."""
        input_name = self.session.get_inputs()[0].name
        
        test_wave_padded = dynamic_pad(test_wave_raw).cpu().numpy()
        
        if self.mode == 'nr':
            score = self.session.run(None, {input_name: test_wave_padded})[0].item()
        elif self.mode == 'ref':
            if ref_wave_raw is None: raise ValueError("ref_wave_raw must be provided for reference mode.")
            ref_wave_padded = dynamic_pad(ref_wave_raw).numpy()
    ...
    def _predict_data_pytorch(self, test_wave_raw, ref_wave_raw=None):
        ...
        with torch.no_grad():
            if self.mode == 'nr':
                ...
                if ref_wave_raw is None: raise ValueError("ref_wave_raw must be provided.")
                ref_wave_padded = dynamic_pad(ref_wave_raw).to(self.device)
