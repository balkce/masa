...
double get_overall_phase_diff(int min_i,int *num_i, double *phases_aligned){
    if (min_i < number_of_microphones-1){
        ...
        return this_diff + get_overall_phase_diff(min_i+1,num_i,phases_aligned);
    }else{
        return 0;
    }
}
...
void apply_weights (jack_ringbuffer_t **in, rosjack_data *out){
    ...
    for(unsigned int j = 1; j < fft_win; j++){
        ...
        double *phases_aligned = (double *) malloc (sizeof(double)*number_of_microphones);
        ...
        if (mag_mean/fft_win > mag_threshold){
          ...
          phase_diff_num = 0;
          phase_diff_sum = get_overall_phase_diff(0,&phase_diff_num,phases_aligned);
