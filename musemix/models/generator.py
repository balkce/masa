...
class MUSE(nn.Module):
    ...
    def forward(self, noisy_mag, noisy_pha, inter_mag, inter_pha): # [B, F, T]
        noisy_mag = noisy_mag.unsqueeze(-1).permute(0, 3, 2, 1) # [B, 1, T, F]
        noisy_pha = noisy_pha.unsqueeze(-1).permute(0, 3, 2, 1) # [B, 1, T, F]
        inter_mag = inter_mag.unsqueeze(-1).permute(0, 3, 2, 1) # [B, 1, T, F]
        inter_pha = inter_pha.unsqueeze(-1).permute(0, 3, 2, 1) # [B, 1, T, F]
        x = torch.cat((noisy_mag, noisy_pha, inter_mag, inter_pha), dim=1) # [B, 4, T, F]
